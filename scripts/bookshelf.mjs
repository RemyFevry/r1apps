...
//     auto                            shelves always update when r1apps updates:
//                                      bump + merge to main + sync every app that
//                                      has bundled documents (run where books live)
//
...
async function main() {
  ...
  if (cmd === 'auto') {
    // Shelves always update when r1apps updates: for every app with bundled
    // documents, land the patch bump on main, then run the normal sync against
    // the merged tree. Bundled docs are local-only (gitignored), so auto must
    // run from a checkout that holds them.
    run('git', ['fetch', 'origin', 'main'])
    const dirty = run('git', ['status', '--porcelain', '--untracked-files=no'], { ok: true })
    if (dirty) die('working tree has tracked changes — commit or stash before auto')
    const original = run('git', ['rev-parse', '--abbrev-ref', 'HEAD'])
    const targets = Object.entries(APPS).filter(([, cfg]) => {
      const dir = cfg.bundleDir
      return existsSync(dir) && readdirSync(dir).some((f) => f.endsWith('.json'))
    })
    if (!targets.length) die('no bundled documents anywhere — nothing to auto-sync')

    for (const [name, cfg] of targets) {
      const pkgRel = relative(ROOT, cfg.pkg)
      const mainVer = parseSemver(JSON.parse(run('git', ['show', `origin/main:${pkgRel}`])).version)
      const refs = run('git', ['ls-remote', '--tags', `https://github.com/${cfg.shelfRepo}.git`])
      const shipped = refs
        .split('\n')
        .map((l) => /^.*\trefs\/tags\/v(\d+)\.(\d+)\.(\d+)$/.exec(l))
        .filter(Boolean)
        .map((m) => ({ major: +m[1], minor: +m[2], patch: +m[3] }))
      const hi = shipped.reduce(
        (a, b) => (b.major > a.major || (b.major === a.major && b.minor > a.minor) || (b.major === a.major && b.minor === a.minor && b.patch > a.patch) ? b : a),
        mainVer,
      )
      const next = `${hi.major}.${hi.minor}.${hi.patch + 1}`
      console.log(`${name}: main v${mainVer.major}.${mainVer.minor}.${mainVer.patch}, shelf v${hi.major}.${hi.minor}.${hi.patch} → v${next}`)

      const br = `shelf/auto-${name}-v${next}`
      run('git', ['checkout', '-B', br, 'origin/main'])
      const pkg = JSON.parse(readFileSync(cfg.pkg, 'utf8'))
      pkg.version = next
      writeFileSync(cfg.pkg, JSON.stringify(pkg, null, 2) + '\n')
      run('git', ['add', pkgRel])
      run('git', ['commit', '-m', `shelf auto: ${name} v${next}`])
      ...
      const pr = run('gh', [
        ...
        'main',
        '--title',
        `shelf auto: ${name} v${next}`,
        '--body',
        'Auto-generated version bump so the shelf can ship this build (a shipped v/<ver>/ is immutable). Generated by `pnpm bookshelf auto`.',
      ]).trim()
      console.log(`  ${pr}`)
      // Auto-merge is disabled on this repo: poll until checks let a direct
      // merge through (failed merge attempts are expected while checks run).
      const num = pr.split('/').pop()
      const deadline = Date.now() + 15 * 60 * 1000
      for (;;) {
        run('gh', ['pr', 'merge', num, '--merge'], { ok: true })
        const state = JSON.parse(run('gh', ['pr', 'view', num, '--json', 'state'])).state
        if (state === 'MERGED') break
        if (state !== 'OPEN' || Date.now() > deadline) die(`PR for ${name} ended up ${state} — resolve manually: ${pr}`)
        execFileSync('sleep', ['10'])
      }
      console.log(`  merged`)
    }

    // Build + sync each app from the merged tree (bundles are local, branches
    // don't carry them — sync reads the working dir, so run it on fresh main).
    run('git', ['fetch', 'origin', 'main'])
  ...
  if (cmd === 'sync') {
    ...
    try {
      ...
      // Both shelf repos (r1-shelf, steady-shelf) are backfilled through their
      // first shipped versions, so every sync from here on ships a release.
      const tag = `v${ver}`
      const releaseTmp = mkdtempSync(join(tmpdir(), 'shelf-rel-'))
      const zipPath = join(releaseTmp, `${APP}-shelf-${tag}.zip`)
